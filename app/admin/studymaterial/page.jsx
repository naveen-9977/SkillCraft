...
export default function studyMaterials() {
  ...
  const updateTitle = (event, index) => {
    let data = [...AllInfos];
    data[0].studyMaterial[index].title = event.target.value;
    setAllInfos(data);
  };

  const updateMentor = (event, index) => {
    let data = [...AllInfos];
    data[0].studyMaterial[index].mentor = event.target.value;
    setAllInfos(data);
  };

  const updateResourceUrl = (event, index) => {
    let data = [...AllInfos];
    data[0].studyMaterial[index].resourceUrl = event.target.value;
    setAllInfos(data);
  };

  const updateResourceType = (event, index) => {
    let data = [...AllInfos];
    data[0].studyMaterial[index].resourceType = event.target.value;
    setAllInfos(data);
  };
