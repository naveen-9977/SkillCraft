...
import '../styles/AdminTests.css'; // Main CSS for the page
import './tests.css'; // New CSS for the batch cards
...
export default function Tests() {
  ...
  const resetForm = () => {
    setFormData({
      ...
      description: '',
      deadline: '',
      batchCode: selectedBatch ? selectedBatch.batchCode : '',
      questions: [{
        questionText: '',
        options: [
  ...
  const fetchTestsForBatch = async (batch) => {
    ...
    try {
        const res = await fetch(`/api/admin/test?batchCode=${batch.batchCode}`);
        const data = await res.json();
        if(res.ok) {
            setTests(data.tests || []);
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSend = { ...formData, batchCode: selectedBatch.batchCode };
      ...
      if (res.ok) {
        setShowForm(false);
        resetForm();
        await fetchTestsForBatch(selectedBatch);
      } else {
        const data = await res.json();
          ...
          <>
            <header className="tests-header">
                <div>
                    ...
                    <p className="tests-subtitle">Select a batch to view or create tests.</p>
                </div>
            </header>
            {error && <div className="error-message">{error}</div>}
            <main className="tests-batch-grid">
                {batches.length > 0 ? batches.map((batch) => (
                    <div key={batch._id} className="tests-batch-card" onClick={() => fetchTestsForBatch(batch)}>
                        <IconFolder />
                        <h2 className="tests-batch-card-title">{batch.batchName}</h2>
                    </div>
                )) : (
                    <div className="empty-state">
                        <h3>No Batches Found</h3>
                        <p>Create a batch to start adding tests.</p>
                    </div>
                )}
            </main>
          </>
        ...
        <>
            <header className="tests-header">
                <div>
                    ...
                    <h1 className="tests-title">Tests for {selectedBatch.batchName}</h1>
                    <p className="tests-subtitle">Create, edit, and manage tests for this batch.</p>
                </div>
